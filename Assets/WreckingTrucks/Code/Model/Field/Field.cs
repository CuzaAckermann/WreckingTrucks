...
public abstract class Field<T> : IClearable, IResetable where T : Model
{
    ...
    public Field(Vector3 position, Vector3 columnDirection, Vector3 rowDirection,
                 int amountColumns, int capacityColumn, Mover<T> modelsMover)
    {
        if (amountColumns <= 0)
        {
    ...
    protected abstract Column<T> CreateColumn(Vector3 position, Vector3 direction, int capacity);
    ...
    private void CreateColumns(int amountColumns, int capacityColumn)
    {
        ...
        {
            _columns.Add(CreateColumn(_position + _rowDirection * i, _columnDirection, capacityColumn));
        }
    }
